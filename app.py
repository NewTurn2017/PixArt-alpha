...
parser = argparse.ArgumentParser(
    description="Gradio App with 8bit and 512model options"
)
...
DESCRIPTION = """Original Source https://pixart-alpha.github.io/ \n
			This APP is modified and brought you by Genie
        """
...
if torch.cuda.is_available():
    ...
    if use_DallE_VAE:
        print("Using DALL-E 3 Consistency Decoder")
        pipe.vae = ConsistencyDecoderVAE.from_pretrained(
            "openai/consistency-decoder", torch_dtype=torch.float16, device_map="auto"
        )
        # speed-up T5
...
def generate(
    ...
    for _ in range(batch_count_int):
        ...
        print(
            f"Image {counter} / {batch_count} - avg image duration {duration:.2f} seconds - step duration {avg_step_duration_ms} ms"
        )  # Print the duration
